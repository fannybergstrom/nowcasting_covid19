...
q_plot_b_30 <- q_plot %>%
  ...
  scale_color_manual(
    values = c(rep(c(wes_cols[2], wes_cols[1]), 3)),
    labels = c(
...
q_plot_d_30 <- q_plot_d %>%
  ...
  scale_color_manual(
    values = c(rep(c(cols[8], cols[1]), 3)),
    # breaks = c("q_05", "q_05_emp", "q_5_emp", "q_5_est","q_95", "q_95_emp"),
    labels = c(
...
ggsave(paste0("./plots/q_plot_modRL.png"), q_plot_d_30,
  width = 3.5,
  height = 3
)
...
ggsave(paste0("./plots/eval_period.png"), q_plot_d_30,
  width = 3.5,
  height = 3
)
...
ggsave(paste0("./plots/scores.png"), fig6,
  width = 3.5,
  height = 3
)
...
err_7 <- matrix(NA, length(N_list), 7)
log_7 <- matrix(NA, length(N_list), 7)
crps_7 <- matrix(NA, length(N_list), 7)
