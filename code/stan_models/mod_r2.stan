...
// log(lambda_{t}) = N(log(lambda_{t-1}) + log(lambda_{t-2}), sigma), 
...
parameters {
  ...
  real<lower=-2, upper=2>  beta_2; // Association coefficient
...
model {
  ...
  for(t in 3:T) {
    logLambda[t] ~ normal( beta_1 * logLambda[t-1] + beta_2 * logLambda[t-2], sigma);
  }
  // Reporting delay
  // Hyper-prior
