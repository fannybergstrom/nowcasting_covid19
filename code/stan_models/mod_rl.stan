...
// log(lambda_{t}) = N(log(lambda_{t-1}) + beta_1 * lead_ind_{t}), sigma), 
...
model {
  ...
  // Random walk
  logLambda[1] ~ normal(log(fmax(sum(r[1,]),1)) + beta_1 * lead_ind[1], 3);
  for(t in 2:T) {
    logLambda[t] ~ normal( logLambda[t-1] + beta_1 * lead_ind[t], sigma);
  }
  // Reporting delay
  // Hyper-prior
