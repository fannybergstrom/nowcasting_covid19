...
// log(lambda_{t}) = N(log(lambda_{t-1}) + beta_1 * lead_ind_1{t} + beta_2 * lead_ind_2{t}), sigma), 
...
model {
  ...
  // Random walk
  logLambda[1] ~ normal(log(fmax(sum(r[1,]),1)) + beta_1 * lead_ind_1[1] + beta_2 * lead_ind_2[1], 3);
  for(t in 2:T) {
